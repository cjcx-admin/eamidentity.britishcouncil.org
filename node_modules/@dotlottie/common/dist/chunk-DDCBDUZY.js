import {
    a as a$1
} from './chunk-74T7T5LL.js';
import {
    a as a$3
} from './chunk-775FELTH.js';
import {
    a
} from './chunk-BHJSY5WG.js';
import {
    p,
    a as a$2,
    j as j$1,
...
var W = (l => (l.Complete = "complete", l.DataFail = "data_fail", l.DataReady = "data_ready", l.Error = "error", l.Frame = "frame", l.Freeze = "freeze", l.LoopComplete = "loopComplete", l.Pause = "pause", l.Play = "play", l.Ready = "ready", l.Stop = "stop", l.VisibilityChange = "visibilityChange", l))(W || {}),
    K = (d => (d.Completed = "completed", d.Error = "error", d.Fetching = "fetching", d.Frozen = "frozen", d.Initial = "initial", d.Loading = "loading", d.Paused = "paused", d.Playing = "playing", d.Ready = "ready", d.Stopped = "stopped", d))(K || {}),
    G = (e => (e.Bounce = "bounce", e.Normal = "normal", e))(G || {}),
    u = {
        autoplay: !1,
        direction: 1,
        hover: !1,
    ...
    j = class {
        ...
        constructor(t, e, i) {
            typeof t == "string" ? this._src = t : this._src = p(t), i != null && i.testId && (this._testId = i.testId), this._defaultTheme = (i == null ? void 0 : i.defaultTheme) || "", this._userPlaybackOptions = this._validatePlaybackOptions(i || {}), typeof(i == null ? void 0 : i.activeAnimationId) == "string" && (this._activeAnimationId = i.activeAnimationId), this._container = e || null, typeof(i == null ? void 0 : i.background) == "string" && this.setBackground(i.background), typeof(i == null ? void 0 : i.activeStateId) < "u" && (this._activeStateId = i.activeStateId);
            let {
                rendererSettings: n,
                ...o
            } = i || {};
        ...
        _setPlayerState(t) {
            ...
            try {
                ...
                j$1(`Invalid PlaybackOptions, ${JSON.stringify(e,null,2)}`);
                return
            }
        ...
        updateSrc(t) {
            this._src !== t && (typeof t == "string" ? this._src = t : this._src = p(t), this._activeAnimationId = void 0, this._currentAnimationId = void 0, this.load());
        }
        get intermission() {
        ...
        goToAndPlay(t, e, i) {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("goToAndPlay() Can't use whilst loading.");
                return
            }
        ...
        goToAndStop(t, e, i) {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("goToAndStop() Can't use whilst loading.");
                return
            }
        ...
        seek(t) {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("seek() Can't use whilst loading.");
                return
            }
            let e = t;
        ...
        _updatePosition(t, e, i$1) {
            let [n, o] = t != null ? t : [0, this.totalFrames - 1], [a, h] = e != null ? e : [0, 1];
            if (!this._areNumbersInRange(a, h)) {
                i("threshold values must be between 0 and 1");
                return
            }
            if (this.container) {
                let {
                    height: f,
                    top: p
                } = this.container.getBoundingClientRect(), d = window.innerHeight - p, m = window.innerHeight + f, l = d / m, _ = n + Math.round((l - a) / (h - a) * (o - n));
        ...
        addIntersectionObserver(t) {
            if (!this.container) throw h("Can't play on show, player container element not available.");
            let e = {
                    root: null,
                    rootMargin: "0px",
        ...
        playOnShow(t) {
            var e;
            if (this.stop(), !this.container) throw h("Can't play on show, player container element not available.");
        ...
        _requireAnimationsInTheManifest() {
            var t;
            if (!((t = this._dotLottieLoader.manifest) != null && t.animations.length)) throw h("No animations found in manifest.")
        }
        _requireAnimationsToBeLoaded() {
            if (this._dotLottieLoader.animationsMap.size === 0) throw h("No animations have been loaded.")
        }
        async play(t, e) {
            var i, n;
            if (["initial", "loading"].includes(this._currentState)) {
                j$1("Player unable to play whilst loading.");
                return
            }
            ...
            if (typeof t == "number") {
                ...
                if (!o) throw h("animation not found.");
        ...
        playSegments(t, e) {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("playSegments() Can't use whilst loading.");
                return
            }
        ...
        resetSegments(t) {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("resetSegments() Can't use whilst loading.");
                return
            }
        ...
        _getAnimationByIdOrIndex(t) {
            ...
            if (this._requireAnimationsInTheManifest(), this._requireAnimationsToBeLoaded(), typeof t == "number") {
                ...
                if (!n) throw h("animation not found.");
                return n
            }
            if (typeof t == "string") {
            ...
            throw h("first param must be a number or string")
        }
        ...
        async _startInteractivity(t) {
            if (!this._inInteractiveMode) {
                i("Can't start interactivity. Not in interactive mode. Call enterInteractiveMode(stateId: string) to start.");
                return
            }
            if (this._dotLottieLoader.stateMachinesMap.size === 0 && await this._dotLottieLoader.getStateMachines(), this._dotLottieLoader.stateMachinesMap.size === 0) throw h("No interactivity states are available.");
            if (t === "undefined") throw h("stateId is not specified.");
            this._stateMachineManager || (this._stateMachineManager = await a$3(Array.from(this._dotLottieLoader.stateMachinesMap.values()), this)), this._stateMachineManager.start(t);
        }
        enterInteractiveMode(t) {
            var e;
            if (t) this._inInteractiveMode || (this._prevUserPlaybackOptions = { ...this._userPlaybackOptions
            ...
            else throw h("stateId must be a non-empty string.")
        }
        exitInteractiveMode() {
            var t;
            this._inInteractiveMode && (this._inInteractiveMode = !1, this._activeStateId = "", (t = this._stateMachineManager) == null || t.stop(), this._userPlaybackOptions = {}, this._userPlaybackOptions = { ...this._prevUserPlaybackOptions
            }, this._prevUserPlaybackOptions = {}, this.reset());
        }
        reset() {
            var i;
            let t = this._getActiveAnimationId(),
                e = (i = this._dotLottieLoader.manifest) == null ? void 0 : i.animations.find(n => n.id === t);
            if (this._inInteractiveMode && this.exitInteractiveMode(), !e) throw h("animation not found.");
            this.play(t);
        }
        previous(t) {
            if (!this._dotLottieLoader.manifest || !this._dotLottieLoader.manifest.animations.length) throw h("manifest not found.");
            if (this._inInteractiveMode) {
                j$1("previous() is not supported in interactive mode.");
                return
            }
            let e = this._dotLottieLoader.manifest.animations.findIndex(n => n.id === this._currentAnimationId);
            if (e === -1) throw h("animation not found.");
            ...
            if (!i || !i.id) throw h("animation not found.");
        ...
        next(t) {
            if (!this._dotLottieLoader.manifest || !this._dotLottieLoader.manifest.animations.length) throw h("manifest not found.");
            if (this._inInteractiveMode) {
                j$1("next() is not supported in interactive mode.");
                return
            }
            let e = this._dotLottieLoader.manifest.animations.findIndex(n => n.id === this._currentAnimationId);
            if (e === -1) throw h("animation not found.");
        ...
        resize() {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("resize() Can't use whilst loading.");
                return
            }
            this._lottie.resize();
        }
        stop() {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("stop() Can't use whilst loading.");
                return
            }
        ...
        pause() {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("pause() Can't use whilst loading.");
                return
            }
        ...
        freeze() {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("freeze() Can't use whilst loading.");
                return
            }
        ...
        unfreeze() {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("unfreeze() Can't use whilst loading.");
                return
            }
        ...
        setAutoplay(t) {
            if (this._requireValidAutoplay(t), !this._lottie || ["loading"].includes(this._currentState)) {
                j$1("setAutoplay() Can't use whilst loading.");
                return
            }
        ...
        toggleAutoplay() {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("toggleAutoplay() Can't use whilst loading.");
                return
            }
        ...
        toggleLoop() {
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("toggleLoop() Can't use whilst loading.");
                return
            }
        ...
        async revertToManifestValues(t) {
            ...
            if (e.includes("activeAnimationId")) {
                let o = (n = this._dotLottieLoader.manifest) == null ? void 0 : n.activeAnimationId,
                    a = this._getAnimationByIdOrIndex(o || 0);
        ...
        addEventListeners() {
            var t;
            if (!this._lottie || ["loading"].includes(this._currentState)) {
                j$1("addEventListeners() Can't use whilst loading.");
                return
            }
            this._lottie.addEventListener("enterFrame", () => {
                var i;
                if (!this._lottie) {
                    j$1("enterFrame event : Lottie is undefined.");
                    return
                }
                ...
                var n;
                if (!this._lottie) {
                    j$1("loopComplete event : Lottie is undefined.");
                    return
            ...
            for (let [e, i] of this._listeners)
                if (e === "complete")
                    for (let n of i)(t = this._container) == null || t.addEventListener(e, n);
                else
                    for (let n of i) this._lottie.addEventListener(e, n);
        }
        ...
        async _getAudioFactory() {
            if (this._animation && m(this._animation)) {
                let {
                    DotLottieAudio: t
                } = await
        ...
        async render(t) {
            ...
            else if (!this._animation) throw h("no animation selected");
            let e = (b = u.loop) != null ? b : !1,
                i = (g = u.autoplay) != null ? g : !1,
                n = (v = u.playMode) != null ? v : "normal",
                o = (P = u.intermission) != null ? P : 0,
                a = (S = u.hover) != null ? S : !1,
                h$1 = (L = u.direction) != null ? L : 1,
                f = (k = u.speed) != null ? k : 1,
                p$1 = (w = u.defaultTheme) != null ? w : "";
            ...
            let d = { ...this._animationConfig,
                    autoplay: a ? !1 : i,
                    loop: typeof e == "number" ? !0 : e,
                    renderer: this._worker ? "svg" : (F = this._animationConfig.renderer) != null ? F : "svg"
                },
                [m, l, _] = await Promise.all([this._dotLottieLoader.getTheme(p$1), this._getLottiePlayerInstance(), this._getAudioFactory()]);
            if (m && this._animation ? (this._animation = p(this._animation), this._animation.slots = m) : this._animation = await this._dotLottieLoader.getAnimation((x = this._currentAnimationId) != null ? x : ""), this._activeStateId && !this._inInteractiveMode) {
            ...
            this.destroy(), this._setPlayerState(() => ({
                ...
                hover: a,
                loop: e
            })), _ ? this._lottie = l.loadAnimation({ ...d,
                container: this._container,
                animationData: this._animation,
                audioFactory: _
            }) : this._lottie = l.loadAnimation({ ...d,
                container: this._container,
                animationData: this._animation
            }), typeof this._lottie.resetSegments > "u" && (this._lottie.resetSegments = () => {
                var V;
                (V = this._lottie) == null || V.playSegments([0, this._lottie.totalFrames], !0);
        ...
        async _getLottiePlayerInstance() {
            var i;
            let t = (i = this._animationConfig.renderer) != null ? i : "svg",
                e;
            if (this._worker) return t !== "svg" && j$1("Worker is only supported with svg renderer. Change or remove renderer prop to get rid of this warning."), e = await
            import ('./lottie_worker-Q23FJ6ZR.js'), e.default;
            switch (t) {
                case "svg":
                    {
                        this._light ? e = await
                        import ('./lottie_light-KMJEUZFY.js') : e = await
                        import ('./lottie_svg-MJGYILXD.js');
                        break
                    }
                case "canvas":
                    {
                        this._light ? e = await
                        import ('./lottie_light_canvas-B5UTTNXA.js') : e = await
                        import ('./lottie_canvas-CDSUBMCL.js');
                        break
                    }
                case "html":
                    {
                        ...
                        import ('./lottie_html-X3TYKVQI.js');
                        break
                    }
                default:
        ...
        async load(t) {
            ...
            try {
                ...
                else if (typeof this._src == "object" && l(this._src)) this._dotLottieLoader.loadFromLottieJSON(this._src);
                else throw h("Invalid src provided");
                if (!this._dotLottieLoader.manifest) throw h("No manifest found");
                let e = this._getActiveAnimationId();
                if (!e) throw h("No active animation found");
                await this._setCurrentAnimation(e), await this.render(t);
            } catch (e) {
                this.setCurrentState("error"), e instanceof Error && i(`Error loading animation: ${e.message}`);
            }
        }
        setErrorState(t) {
            this.setCurrentState("error"), i(t);
        }
        _requireValidDirection(t) {
            if (t !== -1 && t !== 1) throw h("Direction can only be -1 (backwards) or 1 (forwards)")
        }
        _requireValidIntermission(t) {
            if (t < 0 || !Number.isInteger(t)) throw h("intermission must be a positive number")
        }
        _requireValidLoop(t) {
            if (typeof t == "number" && (!Number.isInteger(t) || t < 0)) throw h("loop must be a positive number or boolean")
        }
        _requireValidSpeed(t) {
            if (typeof t != "number") throw h("speed must be a number")
        }
        _requireValidBackground(t) {
            if (typeof t != "string") throw h("background must be a string")
        }
        _requireValidAutoplay(t) {
            if (typeof t != "boolean") throw h("autoplay must be a boolean")
        }
...
export {
    W as a, K as b, G as c, u as d, Q as e, j as f
};
